...
**The document is CNY-only, and that is why cc-core stores a per-row rate.** Wallets are USD-denominated; users pay and read their balance in CNY. Converting at display time leaves every figure floating — the same range exported a week apart totals differently and neither number can be recomputed once the live rate moves. So `requestlog.Record.CNYPerUSD` (cc-core ≥ v0.8.89, migration 4, column `req.cny_rate`) snapshots the rate at settle time, stamped at all six requestlog write sites via `Server.settleCNYRate`. `Record.BilledCNY()` converts a row at its own rate and reports `ok=false` rather than converting at zero when the rate is unknown. The rate, not the converted amount, is stored: rounding per-row yuan to cents makes the rows disagree with the range total, and the debit really was in USD.

Four more things that are load-bearing:

- **The statement reconciles against the ledger, and reports the gap rather than distributing it.** `DB.ChargedUSDBetween` sums what was really debited over the window from **both** `wallet_tx` and `workspace_tx` — a workspace member's spend hits the shared pool first, so reading only `wallet_tx` reports a team member as having spent almost nothing. The debit is a transaction while the itemised row is a separate append to the request log, so the log can lose a line the ledger still holds; `reconcileAgainstLedger` carries that difference as `UnitemisedCNY` and closes the document on three lines (`合计` / `未能明细化的消费` / `区间实际扣款`) instead of one. The gap converts at the range's **own** effective rate (its yuan total over its USD total), not today's, so it stays reproducible like every other figure. Only a shortfall is reported — the log exceeding the ledger means something else (an unbilled attempt row, a refund) and is never shown as a missing charge. Reconciliation failure is logged and skipped, never fatal: it is an enrichment, and a statement that refuses to render because SQLite was briefly busy is worse than one missing a line.
...
- **Both headline figures come from one scan, with the same filter `handleStatusQuery` issues**, so the export rides that endpoint's warm `cachedQueryShared` entry. That also makes the range total and the running total comparable, which they would not be if the latter came from the in-memory `usage` ledger like the panel's Lifetime KPI. The running total is retention-bounded and the PDF says `近 N 天` rather than calling it `累计`.
- **`Statement.Observe` carries the range totals and the per-model table; `Lines` is only what gets printed.** Deriving the rollup from `Lines` left the preview's `by_model` empty and, past `MaxDetailLines` (3000), printed a per-model table summing to less than the total beneath it. Rows predating rate capture fall back to the current rate and are counted in `UnratedRequests`, which the PDF footnotes **only while such rows are in range** — a transitional caveat that removes itself within one retention window rather than becoming permanent furniture. Day labels resolve in `requestlog.BucketLocation()`, not UTC; `statementRange` owns that and is tested, because an eight-hour shift silently moves an evening of requests into the neighbouring month.
