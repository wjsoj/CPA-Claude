...
# Wrap a github.com / raw.githubusercontent.com URL with the mirror prefix if set.
gh_url() {
  if [ -n "$MIRROR" ]; then
    printf '%s%s' "$MIRROR" "$1"
  else
...
msg "downloading $URL"
curl -fsSL --connect-timeout 15 --max-time 300 "${auth_header[@]}" \
  -o "$TMP/$ASSET" "$URL" \
  || err "download failed; check tag/asset name, or try --mirror https://gh-proxy.com/"

msg "verifying checksum"
if curl -fsSL --connect-timeout 15 --max-time 60 "${auth_header[@]}" \
